...
import { dbCreateInvitationFactory } from "../../factory/db/dbInvite.factory";
...
class InviteService {
    ...
    constructor() {
        ...
        async createInvite(data: { email: string; type: string; super_admin_id: number; base_price_up?: number }): Promise<{ success: boolean; data?: any; details?: string }> {
            try {
                const { email, type, super_admin_id, base_price_up } = data;
                
                // Generate a unique invite code (you can use any method you prefer)
                const invite_code = tokenService.generateRandomToken(16);
    
                // Prepare invitation data
                const invitationData = dbCreateInvitationFactory({ email, type, invite_code, super_admin_id, base_price_up });
                ...
                if (result && result.data && result.data.length > 0) {
                    ...
                    const inviteLink = config['inviteLinkFrontendUrl'] + invite_code;
